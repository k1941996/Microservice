...
export default function Form() {
  ...
  const flipLogin = () => {
    return (
      <div className="mt-8 flex justify-center items-center">
        <p className="font-medium text-base">{isLogin ? "Don't have an account?" : "Already have an account?"}</p>
        <button
          onClick={() => {
            setIsLogin(!isLogin);
            setPasswordError("");
  ...
  return (
    <>
      {isLogin ? (
        <Login flip={flipLogin} />
      ) : (
        <div className="bg-white px-10 py-20 rounded-3xl border-2 border-gray-200">
          <h1 className="text-5xl font-semibold">{isLogin ? "Welcome Back" : "Create Account"}</h1>
